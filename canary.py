...
# cargo update shiguredo_libvpx を実行
def run_cargo_update(dry_run: bool) -> None:
    if dry_run:
        print("Dry-run: Would run 'cargo update shiguredo_libvpx'")
    else:
        subprocess.run(["cargo", "update", "shiguredo_libvpx"], check=True)
        print("cargo update shiguredo_libvpx executed")
